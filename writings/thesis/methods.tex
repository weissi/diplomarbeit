...
They represent values and the sub--circuits evaluate to encrypted tuples. In
fact, the sub--circuits are also twofold, they have a left and a right side that
eventually evaluate to two values: Both sides of the encrypted tuple. For
...
and $\beta'$--the same value in all DRAVs throughout the whole process. The
purpose they serve will be discussed in the following sections.
...
describing the methodology.  In the reality especially the function $D$ is never
called by anyone. That would not even be possible since only the party in
...
randomness. As Lemma \ref{lem:well-formed-fun-of-dec-fun} proves, a DRAV is
...
the final multiplication encoding is described in the next section, followed by
...
shared with other DRAVs by chance. For details see Section \ref{def:DRAV}.
...
development which is described in Section \ref{sec:sec-muls}.
...
construction in Section \ref{sec:enc-muls}) is a potential correlation between
...
resulting DRAV $\widetilde{z}$ yield as $\beta_5 = r_1r_2 + r_3 + r_4$ and
...
entire arithmetic function. As discussed in Section \ref{def:DRAE}, not every
...
second party resemble the pre--evaluated DRAEs from Section \ref{def:DRAE}.
...
DRAV to the variable as described in the DRAC\@. As explained in Sections
...
performed by the party that evaluates the DRAEs, it is able to keep track of the
