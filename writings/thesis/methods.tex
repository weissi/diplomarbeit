...
The formal notation (see chapter \ref{sec:DRAE-formal}) of the DRAE $E$  which
specifies the multiplication as described in this chapter is:
...
% FORMAL NOTATION
\JWlthree{Formal Notation}
\label{sec:DRAE-formal}

Because DRAE enable to describe complex compositions multiplications and
additions of DRAVs, they deserve a compact formal notation.


Informally, a DRAE is composed of two finite multi--sets and a
constant DRAV\@. The first multi--set, called the \emph{multiplicative terms}
contains tuples of two \emph{computation radicals}. The second multi--set,
called the \emph{additive terms} contains plain computations. The computations
and computation radicals are references to OAFEs which perform the computations
because the receiver should not learn the specification of the computations but
only one evaluation. To evaluate a DRAE, the receiver starts to replace the
computations (and computation radicals) by their evaluated value. The next step
is to calculate the sum

To evaluate a DRAE, the receiver starts by evaluating the OAFEs which are
referenced as computations and computation radicals. After having evaluated the
Both, computations and computation radicals are

are OAFE evaluations which form a small part of the
large computation a DRAE describes. The sum of two computation radicals

AH FUCK, NEUSCHREIBEN

Each computation is syntactically a
tuple of two \emph{radicals}. The sum of the two \emph{radicals} The constant
...
\JWtodo{Noch natürlichsprachlich hinschreiben wie das geht}
