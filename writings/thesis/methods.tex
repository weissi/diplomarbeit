...
additive ($A$) terms. Notably, even if the encryption keys change, this
...
consisting of many DRAEs. The number of DRAEs will be $O(n)$, $n$ being the
number of arithmetic operations needed by evaluate $f(x)$ as usual. To securely
evaluate the DRAC, the OAFE setup is calculated by the first party. It's
important that the first party accumulates all OAFE calls using the same
variable to one OAFE call resulting in a vector.
...
DRAC. It evaluated the DRAEs one by one and assigns the resulting DRAV to the
