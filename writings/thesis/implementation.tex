...
which are the same as their \emph{dual} counterparts but separated in the tuple
...
TCP/IP networking. The \JWtoken{} implements the OAFE functionality
\JWfuncSym{seq-ot}{OAFE} as provided by the David \& Goliath protocol
\cite{davidgoliath}. The \JWtoken{} does not implement the David \& Goliath
protocol but only its interface. A real implementation using a tamper--proof
hardware token is left open to potential future works.
...
For calculations in finite fields (Chapter \ref{sec:field}), the \JWTcpp{}
library \JWTLntl{} (\emph{Number Theory Library}) has been interfaced to
\JWThaskell{} to use in the implementation of this thesis. The interface
...
directly be interfaced by the FFI.
...
\begin{itemize}
  ...
  \item \JWpath{protos/}, \JWTXprotobuf{} definition files.
