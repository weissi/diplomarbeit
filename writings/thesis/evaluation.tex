...
polynomial's degree, too. The reason is that the implementation has store all
DRAEs to be able to generate die OAFE configuration and to write the OAFE
...
corrupted function evaluating party (\JWpTwo{}), but only secure against
passively corrupted function definition party) arbitrary arithmetic circuits
(include Square \& Multiply) are possible using the techniques presented in
chapter \ref{sed:methods}.

For the main case Oblivious Polynomial Evaluation, this thesis also contributes
...
The implementation would profit by a better implementation of a library for
