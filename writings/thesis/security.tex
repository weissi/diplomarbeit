...
\begin{itemize}
  ...
  \item Setup emulated, honest OAFE functionality labeled
    $\mathcal{F}^R_{OAFE}$.
...
\begin{itemize}
  ...
  \item Intercept \JWpTwo{}'s ($\mathcal{D}^C$) input. This is trivial: It's the
    first message that \JWpTwo{} transmits to the OAFE functionality

  \item Ask the reference \JWpTwo{} $\mathcal{D}^R$ to send the same input to
  ...
  \item The final step analyzes the variable \JWcode{cheated}. If
    \JWcode{cheated $=$ $true$} the emulated OAFE functionality sends uniform
    random to the corrupted $\mathcal{D}^C$. If \JWcode{cheated $=$ $false$},
    use the ideal functionality, evaluate it using \JWpTwo{}'s original input
    and send the result to \JWpTwo{}.
